...
class CoinMarketCap {
   ...
   */
  getTickers (args = {}) {
    console.log('gettickers args',args)
...
module.exports = {
  refreshIds: () => new Promise( ( resolve, reject ) => {
    client.getIdMap().then( result => {
      console.log('status of coinmarket id fetch', result.status)
      resolve( result.data )
    }).catch( reject )    
  }),
  refreshTickers: options => new Promise( ( resolve, reject ) => {
    client.getTickers(options).then( result => {
      console.log('status of get tickers', result.status )
      resolve( result.data )
    }).catch( reject )
  }),
  refreshQuotes: ids => new Promise( ( resolve, reject ) => {
    client.getQuotes({id:ids}).then( result => {
      console.log('status of get quotes', result.status )
      resolve( result.data )
    }).catch( reject )   
  })
